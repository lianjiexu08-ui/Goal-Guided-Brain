...
- **多团队协作**：为开发、运维、产品或不同项目建立独立团队，团队之间通过项目经理受控委派；开启协作的团队默认也能发现之后新建的团队。每次委派都会在源团队和目标团队留下关联记录，并用同一个任务 ID 跟踪进度。
...
团队之间的协作也由项目经理完成。源团队需要开启协作，并允许目标团队；目标团队需要处于已确认且活动状态。目标团队会用自己的项目经理、模型、能力、预算和工作目录创建独立任务，源团队动态会保留委派记录并显示目标任务状态。通过网页 API 或编排 MCP 发起的委派使用同一套权限和幂等规则，重复提交不会创建重复任务。
