...
- **路由可解释**：任务会保留实际供应商、模型、候选能力和自动回退事件，便于判断为什么选中或切换模型。
- **能力管理**：安装、固定、启用和回退 Skill、MCP 与 Plugin；智能体只能使用已绑定能力。
- **可恢复执行**：任务组、子任务、消息、检查点、产物、预算、租约和远程节点状态都会持久化。
- **团队工作区隔离**：团队默认在隔离目录执行；创建团队时可以明确选择 Git worktree、快照或共享目录。
...
`npm run test:e2e` 会启动隔离的 API/UI 服务，在桌面和移动 Chromium 尺寸下验证团队招募、草稿恢复、页面切换、模型切换、失败任务恢复和真实二进制附件；后端还包含两个团队连续 20 次交替消息的归属压力测试。首次运行若本机没有 Playwright 浏览器，请执行 `npx playwright install chromium`。测试使用临时数据库、本地确定性模型和 MCP fixture，不调用付费模型。每次功能迭代都应补充对应验证，检查 `git diff --check`，提交清晰的 Git commit，并推送到远程仓库：
